...
final class WebCoordinator: NSObject {
    ...
    private let cookieJar = "marineroute_cookies"
    
    func loadURL(_ url: URL, in webView: WKWebView) {
        print("🌊 [MarineRoute] Load: \(url.absoluteString)")
        redirectCount = 0
...
extension WebCoordinator: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        if let current = webView.url { checkpoint = current; print("✅ [MarineRoute] Commit: \(current.absoluteString)") }
    }
